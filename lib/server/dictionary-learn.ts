...
export function learnFromMeal(userId: string, meal: MealEntry, isNew: boolean): void {
  // A: 申告名(hint)が公式辞書に無ければ、hint名＋合計栄養を学習（新規記録時のみ）
  if (
    isNew &&
    validName(meal.hintName) &&
    meal.nutrition &&
    !lookupOfficialByName(meal.hintName)
  ) {
    upsertFood(
      userId,
      { name: meal.hintName, nameJa: meal.hintName, ...nutritionOf(meal.nutrition) },
      true
    );
  }
