# ごはんくん 画像

`GohankunWidget` はこのフォルダの PNG を参照します。
添付の5枚を、下の対応で保存してください（未配置でも簡易SVGで表示されます）。

| ファイル名 | 状態 (`state`) | 添付画像（どれを保存するか） |
| --- | --- | --- |
| `gohankun-happy.png` | `happy` | にっこり笑顔＋まわりに料理（寿司・味噌汁・団子）の絵 |
| `gohankun-thinking.png` | `thinking` | むすっと集中した顔＋湯気の絵 |
| `gohankun-proud.png` | `proud` | 日本国旗を掲げてガッツポーズの絵 |
| `gohankun-eating.png` | `eating` | ウインクしてお箸で寿司を食べている絵 |
| `gohankun-explaining.png` | `explaining` | メガネをかけて巻物（手紙）を持っている絵 |

- 推奨: 正方形・透過PNG（背景透過だと一番きれい）。512px 前後でOK。
- 差し替えるだけで反映されます（コードの変更は不要）。
